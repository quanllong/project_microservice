...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 测试当面付2.0生成支付二维码
        // main.test_trade_precreate();    // 369行
    }
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("G:\\kkkk\\qrcode/qr-%s.png", response.getOutTradeNo());
     ...
     */
    // quanllong
    public MainInfo generateCode(String orderId){

        // 查出订单
        List<MoocOrderT> moocOrderTS = this.moocOrderTMapper.selectList(new EntityWrapper<MoocOrderT>().eq("UUID",orderId));
        int price = moocOrderTS.get(0).getOrderPrice();
        System.out.println("price222222222222222222 = " + price);
        System.out.println("111111111111111111" + orderService);

        // 查出电影名字
        List<MtimeFilmT> mtimeFilmTS = mtimeFilmTMapper.selectList(new EntityWrapper<MtimeFilmT>().eq("UUID", moocOrderTS.get(0).getFilmId()));
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", filmName, price, 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
...
//        goodsDetailList.add(goods2);
        ...
        if(fileName == null){
            fileName = "fileNameNotCreated";
        }
        if(filePath == null){
            filePath = "filePathNotCreated";
        }
        log.info("main - " + fileName);
        log.info("main - " + filePath);
