...
public class Main {
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String orderId = " ";
        String outTradeNo = "abcde" + orderId;


        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店消费”
        String subject = "澳门赌场消费，等你来消费";
        ...
        // 支付超时，线下扫码交易定义为 X分钟
        String timeoutExpress = "30m"; /*30minute*/

        // 商品明细列表，需填写购买商品详细信息，
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", "大小点赌局", 1000, 1);
        // 创建好一个商品后添加至商品明细列表
    ...
    public void test_trade_query() {
        ...
        String outTradeNo = "tradeprecreate15752963634087717536";
    ...
    public void test_trade_precreate() {
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", "果冻", 1000, 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径：二维码生成的路径
                Random random = new Random();
                int i = random.nextInt(100);
                String filePath = String.format("D:\\git\\project_microservice\\QRcode/qr-%s.png",
                    response.getOutTradeNo(), i);
